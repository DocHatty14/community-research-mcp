...
def detect_all_capabilities() -> SystemCapabilities:
    ...
    search_capabilities = {
        ...
        SearchCapability.LOBSTERS.value: True,  # No key needed
        SearchCapability.DISCOURSE.value: True,  # No key needed
...
def format_capabilities_report(capabilities: SystemCapabilities) -> str:
    ...
    report.append(f"\n**Total Active Search APIs:** {len(active_search)}")
    report.append(
...
def format_streaming_update(
    ...
    if per_source:
        ...
        for source, stats in per_source.items():
            ...
            output.append(f"- {label}: {stats.get('total', 0)} items ({shape_text})")
        output.append("")
        output.append(
