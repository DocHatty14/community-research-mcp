...
class QualityScorer:
    ...
    def __init__(self, preset: str = "balanced"):
        ...
        self.scoring_weights = self._PRESETS.get(
            self.preset, self._PRESETS["balanced"]
        )["weights"]
    ...
    def score_finding(self, finding: Dict[str, Any]) -> int:
        ...
        # Boost score based on relevance if available
        relevance = finding.get("relevance_score", 0)
        if relevance > 0:
            # Add up to 20 points for highly relevant results
            relevance_boost = (relevance / 100) * 20
            total_score += relevance_boost
...
def deduplicate_results(
    ...
    for source, results in search_results.items():
        for result in results:
            ...
            normalized_title = (
                _normalize_title(result.get("title", "")) if result.get("title") else ""
            )
            ...
            scored_result["quality_score"] = scored_result.get(
                "quality_score"
            ) or scorer.score_finding(scored_result)

            existing = best_by_key.get(dedupe_key)
            if (
                not existing
                or scored_result["quality_score"] > existing["quality_score"]
            ):
